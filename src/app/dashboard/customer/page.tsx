...
export default function UsersPage() {
      ...
      if (modalType === "add") {
        ...
        Object.keys(data).forEach(key => {
          ...
          if (key === 'profile_picture' && value) {
            ...
            value.forEach(permission => formData.append('user_permissions', permission));
  ...
  const getRoleBadge = (user: CustomUser) => {
    ...
    if (user.is_biller) return "bg-orange-100 text-orange-800 border-orange-200";
                    ...
                    {users.length === 0 ? (
                      ...
                      users.map((user,index) => (
                        <tr key={user.id} className="hover:bg-gray-50 transition-colors duration-150">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getRoleBadge(user)}`}>
                              {getUserRole(user)}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusBadge(user.is_active)}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">
                    {modalType === "add" ? "Create New User" :
                     ...
                     modalType === "view" ? "User Details" : "Delete User"}
                  </h2>
                </div>
                <button
                  onClick={handleClose}
              ...
              {modalType === "view" && selected && (
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{getDisplayName(selected)}</h3>
                      <p className="text-gray-600">{selected.email}</p>
                      <div className="flex gap-2 mt-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getRoleBadge(selected)}`}>
                          {getUserRole(selected)}
                        </span>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getStatusBadge(selected.is_active)}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="mt-2 space-y-2">
                          {selected.first_name && (
                            <p><span className="font-medium">First Name:</span> {selected.first_name}</p>
                          )}
                          {selected.middle_name && (
                            <p><span className="font-medium">Middle Name:</span> {selected.middle_name}</p>
                          )}
                          {selected.last_name && (
                            <p><span className="font-medium">Last Name:</span> {selected.last_name}</p>
                          )}
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="mt-2 space-y-2">
                          <p><span className="font-medium">Joined:</span> {new Date(selected.date_joined).toLocaleDateString()}</p>
                          {selected.last_login && (
                            <p><span className="font-medium">Last Login:</span> {new Date(selected.last_login).toLocaleDateString()}</p>
                          )}
                        </div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="mt-2 space-y-1">
                          {selected.user_permissions?.map((permission, index) => (
                            <div key={index} className="flex items-center gap-2">
                              <Check className="w-4 h-4 text-green-600" />
                              <span>{permission}</span>
                            </div>
                          ))}
                          {(!selected.user_permissions || selected.user_permissions.length === 0) && (
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex-1">
                      <div className="mb-4">
                        <h3 className="text-lg font-semibold text-gray-900">{getDisplayName(selected || { email: '', first_name: '', last_name: '' } as CustomUser)}</h3>
                        <p className="text-gray-600">{selected?.email || "New user"}</p>
                      </div>
                      <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {(modalType === "add" || (modalType === "edit" && formValues.password)) && (
                      <div className="md:col-span-2">
                        <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" {...register("is_active")} className="rounded border-gray-300 text-purple-600 focus:ring-purple-500" />
                      <span className="text-sm font-medium text-gray-700">Active User</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" {...register("is_superuser")} className="rounded border-gray-300 text-purple-600 focus:ring-purple-500" />
                      <span className="text-sm font-medium text-gray-700">Superuser</span>
                    </label>
                  </div>

                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-3">User Groups</label>
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-3">User Permissions</label>
                  ...
                  >
                    {button_clicked ? (
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {modalType === "add" ? "Creating User..." : "Updating User..."}
                      </div>
                    ) : (
              ...
              {modalType === "delete" && selected && (
                <div className="text-center">
                  ...
                  <p className="text-gray-600 mb-6">
                    Are you sure you want to delete user <strong>{getDisplayName(selected)}</strong> ({selected.email})?
